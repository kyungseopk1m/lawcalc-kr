...
```json
{
  ...
  "payload": {
    "appVersion": "0.3.0",
    "createdAt": "2026-05-11T12:34:56+09:00",
    "input": {
      "principal": 10000000,
...
- v0.1.x 의 이자 전용 `schemaVersion: "1"` 파일은 로드 시 v2 의 `kind: "interest"` envelope 로 마이그레이션되고, 이어서 v3 envelope (`envelopeFeatures: ["interest@1"]`, `dataVersions.interest` hoist) 으로 추가 마이그레이션됩니다. v2 → v3 마이그레이션은 `kind` 와 capability id, dataVersions 키가 모두 일치하도록 hoist 합니다.
