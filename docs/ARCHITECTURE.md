...
lawcalc-kr 는 한국 법률 계산을 로컬에서 다루는 데스크톱 앱입니다. 현재 버전은 판결금 이자·지연손해금, 상속분 간이 계산, 소송비용 산정, 변제충당, 손해배상(자동차·산재, 부상·사망 및 기타손해) 다섯 도메인을 다룹니다.
...
UI 는 표시용으로 숫자·날짜를 포매팅할 수 있지만, 계산에 영향을 주는 반올림과 일수 선택은 반드시 엔진 결과에서 가져와야 합니다. 이렇게 해야 PDF/CSV/클립보드 출력이 화면 표와 항상 일치합니다.
...
이 포맷은 네트워크 없이 파일을 읽고 씁니다. 하위 호환 reader 는 가능하면 알 수 없는 필드를 보존합니다.
