...
공개 매뉴얼을 참고한 독립 구현이다. 출처 상세: [LEGAL_REFERENCES.md](LEGAL_REFERENCES.md).
...
> **알려진 한계 (1년 초과 + `actual`)** — `mode="totalDays"` + `leapYear="actual"` +
> 구간 1년 초과 케이스에서, 본 엔진은 분모를 단일 (365 또는 366) 로 확정한다.
> 그러나 정확한 비례 계산은 윤일 포함 일수만 366으로, 그 외는 365로 분리해야 한다
> (예: 730일 구간에 윤일 1개 포함 → 1일분만 1/366, 나머지 729일분은 1/365).
> 본 엔진의 단일 분모 채택은 채무자에게 미세 유리(또는 불리)한 오차를 발생시킬
> 수 있으므로, 1년 초과 구간 + 윤년 정확 계산이 필요한 경우 `mode="period"`
> 사용을 권장한다 (TIER-A #3 docs caveat — v0.1.x scope).

### 3.1 출력 표기 정책 (`segment.formula`)

`InterestSegment.formula` 는 계산 근거를 사람이 읽도록 한 줄 문자열로 노출한다.
B-W5 사용자 컨펌으로 **한글 단위 + 백분율** 표기로 통일한다 (UI / PDF / CSV 모두 동일):

| 형태               | 표기 예                                                   |
| ------------------ | --------------------------------------------------------- |
| 총일수식           | `1,000,000원 × 5% × 100일 / 365`                          |
| 기간식 (풀 1년)    | `1년 × 1,000,000원 × 5%`                                  |
| 기간식 (풀 + 미만) | `1년 × 1,000,000원 × 5% + 1,000,000원 × 5% × 100일 / 365` |
| 일수 0             | `1,000,000원 × 5% × 0일 = 0`                              |

규칙:

- 원금: `principal.toLocaleString("en-US")` + `원` (예: `1,000,000원`).
- 이율: `rate × 100` 의 부동소수점 잡음을 `Number(x.toFixed(10))` 로 정리한 뒤 `%` 접미사
  (예: `0.05 → "5%"`, `0.123 → "12.3%"`, `0 → "0%"`).
- 일수: 정수 + `일` 접미사. 분모는 단위 표기 없이 정수만 (`/ 365`, `/ 366`).
- 풀 1년 단위: `${년수}년 × …` 접두사. 풀 + 미만은 `" + "` 로 연결.
- 본 표기는 출력 전용 — 계산 결과(`interest` / `totalInterest`) 에는 영향 없다.
...
### 4.1 1년 만기 정의 — 민법 159·160조 (단일 출처)

풀 1년 cycle 의 만료일은 민법 159·160 통설을 따른다 (`periodCycleEnd` 헬퍼).

> **민법 제159조 (기간의 만료점)** 기간을 일, 주, 월 또는 연으로 정한 때에는
> 기간말일의 종료로 기간이 만료한다.
>
> **민법 제160조 (역에 의한 계산)** ① 기간을 주, 월 또는 연으로 정한 때에는
> 역에 의하여 계산한다. ② 주, 월 또는 연의 처음으로부터 기간을 기산하지
> 아니하는 때에는 최후의 주, 월 또는 연에서 그 기산일에 해당한 날의 전일로
> 기간이 만료한다. ③ 월 또는 연으로 정한 경우에 최종의 월에 해당일이 없는
> 때에는 그 월의 말일로 기간이 만료한다.

본 엔진의 cycle 만료 결정:

| `cursor` (cycle 시작)     | `addYears(cursor, 1)`    | cycle 만료일                | 다음 cycle 시작 (= 만료 + 1일) |
| ------------------------- | ------------------------ | --------------------------- | ------------------------------ |
| `2023-05-01` (일반)       | `2024-05-01`             | `2024-04-30` (전일)         | `2024-05-01`                   |
| `2024-02-29` (윤년 02-29) | `2025-02-28` (clip)      | `2025-02-28` (그 월의 말일) | `2025-03-01`                   |
| `2020-02-29`              | `2021-02-28` (clip)      | `2021-02-28`                | `2021-03-01`                   |
| `2024-02-29`              | `2028-02-29` (4년, 윤년) | `2028-02-28` (전일)         | `2028-02-29` (= 다음 윤일)     |

윤년 02-29 시작 + 비윤년 1년 후 만기 케이스에서, `addYears` 가 02-28 로 clip 한
결과가 그대로 cycle 만료일이며 (160조 3항 "그 월의 말일"), 다음 cycle 의 시작은
03-01 이 된다. 이 정책은 "[2024-02-29, 2025-02-28] = 정확히 1년" 이라는 통설과
일치하며, partial 0 / fullYears=1 로 분해된다 (`case-009` 회귀로 고정).

**의사결정 출처 (TIER-A #2)**: A안 (민법 통설) 채택. 변호사 답변 절차는
`docs/plans/tier-a-2-lawyer-question.md` (for-claude 보관소). B안 (캘린더 정밀
365/366) 과 C안 (1년 + 1일 산정) 은 채택하지 않음.
...
| `case-008` | totalDays | fixed365 | 외부 캡처 대기 슬롯 (`golden-pending/`, ejpc/Windows VM)        |
| `case-009` | period    | actual   | 윤년 02-29 시작 + 02-28 만료 = 정확 1년 (TIER-A #2 A안)         |
| `case-010` | period    | actual   | 윤년 02-29 시작 + 1년 + 15일 partial (clip 흡수 + 03-01 cursor) |

`case-007` 만 외부 매뉴얼 인용이고, 나머지는 엔진 내부 회귀 테스트(`source: engine-internal-*`).
