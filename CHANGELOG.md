...
- 앱 아이콘과 화면 상단 마크를 새로 그렸습니다. 종전의 작은 `Lc` 와 밑줄, 금색 강조 대신 그래파이트 바탕에 오프화이트 `L` 과 코발트 `C` 를 놓아 16px 에서도 글자 모양이 남게 했습니다. 글자는 글꼴이 아니라 고정 윤곽으로 저장해 설치된 글꼴에 따라 결과가 달라지지 않습니다. 계산 동작에는 영향이 없습니다.

## [0.12.0] - 2026-08-18

> **주의. 같은 입력에 v0.11.0 과 다른 금액이 나옵니다.** 이번 릴리스는 정확성 수정이 대부분이라 이자·상속분·소송비용·변제충당·손해배상 전 도메인에서 결과가 달라질 수 있습니다. 진행 중인 사건에 v0.11.0 결과를 이미 제출했다면 다시 계산해 확인해 주세요. 버전 번호는 0.x 규칙에 따라 minor 로 올라가지만 금액이 바뀌는 폭은 그보다 큽니다.

### Added

- 상속인마다 상속포기(민법 제1041조)를 체크할 수 있습니다. 포기는 제1001조의 대습원인이 아니므로 대습상속인이 생기지 않습니다. 어느 촌수 전원이 포기하고 배우자가 있으면 배우자가 단독상속합니다(대법원 2023. 3. 23.자 2020그42 전원합의체). 대습상속인 각자의 포기도 따로 넣을 수 있습니다.
- 대습이 생긴 원인을 상속개시 전 사망, 상속결격(제1004조), 상속권 상실선고(제1004조의2)로 구분해 입력합니다.
- 변제충당에 변제일을 넣을 수 있습니다. 그 날짜를 기준으로 변제기 도래 여부를 판정합니다(민법 제477조 제1호). 비워 두면 종전처럼 계산 시점을 기준으로 봅니다.
- 변제충당 결과에 채권별 충당 순위를 표시합니다. 변제기 도래 여부와 변제이익 순위, 동순위 비례 안분 여부가 함께 나옵니다.
- 소송비용에 소가 산정 기준을 추가했습니다. 소가를 산출할 수 없는 재산권상의 소와 비재산권을 목적으로 하는 소송은 5,000만원으로, 「민사소송 등 인지규칙」 제15조 제1항부터 제3항까지와 제15조의2·제17조의2·제18조에 정한 소송은 1억원으로 간주합니다(제18조의2).
- 항고·재항고 사건에 원신청서 인지액을 넣을 수 있습니다. 제9조·제10조의 신청에 관한 재판에 대한 항고는 그 2배이고(「민사소송 등 인지법」 제11조 제1항), 비워 두면 제11조 제2항의 2,000원 정액이 적용됩니다.
- 향후치료비·보조구에서 같은 지출을 여러 항목으로 나눠 넣은 것으로 보이는 조합을 찾아 안내합니다. 수치합계 한도가 항목마다 따로 걸려 합계가 커지기 때문입니다. 금액은 바꾸지 않고 표시만 합니다.
- 한도가 걸려 금액이 줄었다는 사실과 분할 의심 안내가 클립보드·PDF·CSV 에도 나갑니다. 이전에는 화면에만 있어서, 실무에서 최종 산출물이 되는 PDF 산출근거에서 빠졌습니다. 소송비용의 대한법률구조공단 적용 범위 경고도 같습니다.
- 소촉법 이율을 골랐을 때 계산 기간이 이율 변경일(2015-10-01, 2019-06-01)을 걸치면 부칙 경과조치를 안내합니다. 변론종결일은 입력에 없으므로 계산은 그대로 두고 안내만 합니다.
- 저장하지 않은 계산 내용이 있는 상태로 창을 닫으면 확인 창이 뜹니다.
- 상단 탭을 화면 낭독기가 탭으로 인식하도록 바꾸고, 활성 탭을 색 외에 밑줄로도 표시합니다.

### Fixed

- 여러 구간으로 나눈 이자 계산에서 초일 불산입이 구간마다 중복 적용돼 구간 수만큼 일수가 빠졌습니다. 2구간은 363일(정답 364일), 3구간은 2,189일(정답 2,191일)이었습니다. 초일 불산입은 기간의 기산일에 한 번 적용하는 규칙이고(민법 제157조) 구간 분할은 이율 문제라 무관합니다.
- 이율에 유효하지 않은 값이 들어가면 검증 없이 통과해 이자가 계산 불능 값으로 나왔습니다. 원금은 이미 막혀 있어 대칭이 깨져 있었습니다.
- 소촉법 이율을 골랐는데 계산 기간이 2003-06-01 이전까지 걸치면 함수 이름이 든 영어 오류 문구가 결과 화면에 그대로 떴습니다. 이제 계산 전에 한국어로 안내하고 계산을 막습니다. 그 이전 기간에 소촉법 이율이 없는 이유는 데이터 누락이 아니라 헌재 2003. 4. 24. 선고 2002헌가15 위헌결정으로 구 규정이 실효됐기 때문이며, 안내 문구에 그 사유를 밝힙니다.
- 민법·상법 법정이율의 적용 시작일이 시행일이 아니라 공포일로 들어가 있었습니다. 1958년, 1959년, 1962년 기산 계산의 결과가 달라집니다.
- 이자제한법 데이터셋의 대통령령 번호 3건을 정정했습니다.
- 1순위 직계비속에 최근친 우선(민법 제1000조 제2항)이 적용되지 않아, 자녀와 손자녀를 함께 넣으면 손자녀에게도 지분이 갔습니다.
- 2026. 3. 17. 공포·시행된 법률 제21454호가 제1003조 제2항의 대습 배우자 범위를 사망 또는 결격된 자의 배우자에서 상속개시 전에 사망한 사람의 배우자로 좁힌 것을 반영합니다. 시행일 이후 개시된 상속에서 결격이나 상속권 상실이 원인이면 피대습자의 배우자는 대습분을 받지 못하고, 직계비속 대습은 그대로 유지됩니다. 부칙 제2조의 적용례에 제1003조가 없어 시행일로 갈립니다.
- 같은 상속인에게 상속개시 전 사망과 상속포기를 함께 체크하면 입력해 둔 대습상속인이 경고 없이 결과에서 빠졌습니다. 최악의 경우 배우자 단독상속으로 뒤집혔습니다. 두 상태를 함께 지정할 수 없게 했습니다.
- 항소·상고 인지액이 불복 범위를 반영하지 않고 전체 소가로 계산됐습니다. 전체 소가 1억원에 불복 범위 3,000만원인 사건에서 3.25배로 과대했습니다(「민사소송 등 인지규칙」 제25조).
- 항소·상고 사건을 저장하면 전체 소가가 파일에 남지 않아, 다시 열어 심급을 1심으로 되돌리면 불복 범위를 소가로 착각해 조용히 과소 계산됐습니다.
- 조정신청에 인지법 제2조 소장 산식이 그대로 적용돼 10배로 나왔습니다. 「민사조정규칙」 제3조의 10분의 1을 적용합니다.
- 조정신청에 항소·상고 심급을 지정할 수 있어 존재하지 않는 상소 수수료가 산출됐습니다. 소가 3,000만원 기준으로 항소 21,000원, 상고 28,000원이었습니다. 조정 신청 수수료에는 심급 개념이 없어 1심으로 고정합니다.
- 항고·재항고 인지를 소장 누진식으로 계산했습니다. 인지법 제11조의 별도 체계를 적용합니다.
- 항고 사건의 원신청서 인지액에 음수나 유효하지 않은 값을 넣어도 검증 없이 통과해, 음수 인지액이 결과에 실렸습니다.
- 전자소송 감액(인지법 제16조)이 1,000원 하한에 먹혀 무효화되던 구간이 있었습니다. 하한을 심급 배수보다 앞에 두도록 순서를 바로잡아, 소가 10,000원 사건의 항소가 1,000원(정답 1,500원)으로, 상고가 1,000원(정답 2,000원)으로 나오던 것도 함께 고쳤습니다.
- 송달료 회당 단가가 낡아 있었습니다. 2026-07-01 시행 단가 5,640원을 적용합니다.
- 가압류·가처분 이의·취소 신청사건의 변호사보수가 0원으로 강제됐습니다. 「변호사보수의 소송비용 산입에 관한 규칙」 제3조 제2항 단서의 변론·심문 요건은 신청사건 전용이라 이의·취소에는 적용되지 않습니다.
- 소가 산정 기준 값에 오타가 들어가면 검증 없이 간주 소가로 처리돼, 소가 10억원 사건의 인지액이 4,055,000원에서 230,000원으로 바뀌었습니다.
- 화면을 초기화해도 소가 산정 기준이 그대로 남아 기본 화면에서 230,000원이 계산됐습니다. 정상값은 140,000원입니다.
- 불복 범위가 소가와 따로 움직여, 소가만 고치면 불복 범위가 옛 값에 남았습니다. 이제 비워 두면 소가를 따라가고, 간주 소가를 고르면 함께 비활성됩니다.
- 동순위 채권의 비례 안분에서 변제금이 사라졌습니다. 변제 5원에 총채무 6원이면 4원만 충당됐고, 채권 5건 74원은 71원이 됐습니다. 배분 결과가 채권 입력 순서에 따라 달라지기도 했습니다.
- 채권 잔액에 안전정수 범위를 벗어난 값을 넣어도 검증 없이 통과했습니다.
- 사고 당시 만 25세 미만이면 손해배상 계산 자체가 불가능했습니다. 만 24세, 만 10세, 신생아 모두 오류였고 가동연한을 65세보다 높게 잡아도 같았습니다.
- 가동연한이 이미 지난 피해자는 위자료만 청구해도 계산이 거부됐습니다.
- 기왕증 기여도를 입력해도 일실수입에 반영되지 않았습니다. 입력란과 저장까지 배선돼 있는데 계산만 이 값을 읽지 않아 조용히 과대한 금액이 나왔습니다.
- 극단적으로 짧은 한시장해가 경고 없이 사라졌습니다.
- 같은 지출을 나눠 넣은 조합을 찾을 때 기간이 부분적으로 겹치거나 한쪽이 다른 쪽에 포함되는 경우를 놓쳤습니다.
- 계산 결과 화면의 조문 표기에서 절 기호를 없애고 제N조 제N항 형태로 통일했습니다.

### Changed

- 상속포기와 대습 원인, 변제일, 소가 산정 기준과 전체 소가, 원신청서 인지액이 담긴 `.lcalc` 파일은 `inheritance@2`, `appropriation@2`, `litigation-cost@2` 를 요구합니다. 이 필드가 없는 파일은 종전대로 저장되어 이전 버전에서도 열립니다. 반대로 새 필드가 담긴 파일을 이전 버전에서 열면 필드가 조용히 버려져 금액이 달라지므로, 이전 버전이 열지 못하게 막습니다.
- 데이터셋 출처 메타데이터(`legal-rates`, `interest-limits`, `delivery`, `stamp-duty`)를 정정했습니다.
...
- 지급명령 인지 감액(제7조 제2항)을 사건구분만으로 적용합니다. 이전에는 사건구분을 독촉사건으로 고르고도 별도 체크박스를 놓치면 소장 인지액이 그대로 나왔습니다. v0.10.0 이하에서 저장한 파일을 열어 다시 계산해도 감액이 유지됩니다.
- 소송비용에 산입하는 변호사보수를 실제 지급보수액 범위 내로 제한합니다(「변호사보수의 소송비용 산입에 관한 규칙」 제3조 제1항). 지급보수액 입력란을 새로 뒀고, 넣지 않으면 결과가 별표 상한액이라는 점을 산식에 함께 표시합니다.
- 가압류·가처분 사건의 변호사보수를 별표 산정액의 2분의 1로 계산합니다(제3조 제2항). 신청사건에서 변론이나 심문을 거치지 않았으면 산입하지 않습니다. 이전에는 반대로 계산해, 변론을 거친 사건에 별표 전액이 나오고 거치지 않은 사건에 2분의 1이 나왔습니다.
- 보전처분 인지에는 100원 절사를 적용하지 않습니다. 제7조 제4항은 화해·지급명령에 대해 제2조 제2항을 준용한다고 밝혔지만 제9조에는 그런 규정이 없습니다. 소가 1,100만원 임시지위 가처분은 27,250원입니다.
- 이전 버전에서 보전처분에 항소심이나 화해를 함께 저장한 `.lcalc` 파일이 열리지 않던 문제를 고쳤습니다. 파일을 열 때 1심 단독 신청으로 맞춰 읽습니다.
- 인지액 산식 표시가 절사 전 금액을 건너뛰어 계산이 맞지 않아 보이던 것을 고쳤습니다.
- 금액 입력에서 소수점 이하를 버립니다. 이전에는 소수점만 지우고 숫자를 이어 붙여 "30,000,000.00"을 3,000,000,000원으로 읽었습니다.
...
- 산재 손해배상의 **산재보험급여 공제**를 대법원 2022. 3. 24. 선고 2021다241618 전원합의체 판결에 맞췄습니다. 장해급여·유족급여를 같은 성질의 손해인 **일실수입 한도에서 먼저 공제한 다음 과실상계**하며("공제 후 과실상계"), 급여가 일실수입을 넘어도 위자료·장례비·기타손해에서는 공제하지 않습니다. 이전에는 과실상계 후 총액에서 전액을 차감해 과실 사건은 적게, 급여가 큰 사건은 위자료까지 잠식해 계산됐습니다.
- 변제충당에서 합의·지정 충당 대상보다 변제액이 클 때 **잉여를 민법 제477조 법정충당 순서로 잔여 채권에 충당**하도록 바로잡았습니다. 이전에는 같은 채권에 잔액이 남아 있어도 잉여가 어디에도 충당되지 않았습니다. 모든 채권이 소진된 뒤 남는 금액만 잉여(반환)로 표시합니다.
- 소송비용 안분(소가비례) 분배의 잔여 원 단위를 **최대잉여법**으로 배정합니다. 이전에는 입력 순서대로 앞 당사자부터 배정해 당사자 순서를 바꾸면 분배액이 달라질 수 있었습니다. 이제 소수 잉여가 큰 당사자부터 받고, 순서를 바꿔도 같은 당사자가 같은 금액을 받습니다.
...
- **사건 파일**을 추가했습니다. 상단에 사건번호·사건명을 적고 "사건 저장"을 누르면 입력해 둔 탭들의 계산이 하나의 `.lcalc`(`case@1`)로 묶여 저장되고, "사건 열기"로 각 탭이 그대로 복원됩니다. 단일 계산 파일을 사건 열기로 열면 해당 탭으로 이동합니다. 입력 오류가 있는 탭이 있으면 저장을 멈추고 어느 탭인지 알려 주며, 구버전 앱에서 사건 파일을 열면 어떤 기능이 필요한지 안내하고 거부합니다.

### Changed

- 탭을 옮겨 다녀도 입력하던 값이 유지됩니다. 이전에는 다른 탭에 다녀오면 입력이 사라졌습니다.
- 카드 제목과 같은 말을 반복하던 설명 문장을 덜어내고 화면 간격을 줄여, 입력과 결과가 한 화면에 더 가깝게 들어오도록 정리했습니다. 조문·데이터 근거 안내는 그대로 둡니다.
- 변제충당의 충당 방식 선택 아래에 우선순위(합의 → 지정 → 법정) 안내를, 소송비용의 변호사보수 옵션에 근거 규칙명(「변호사보수의 소송비용 산입에 관한 규칙」)을 추가했습니다.
...
- 상속인 입력 UI를 공용 컴포넌트로 분리해 상속분 탭과 손해배상 사망 모드가 같은 입력을 사용하도록 정리했습니다.

### Notes

- 자×사망의 상속 분배는 상속분 간이 계산 엔진을 재사용하므로 1991-01-01 이후 사망 케이스를 대상으로 합니다. 1990년 이전 사망 케이스는 후속 검토 범위입니다.
- 기존 자×부상 `.lcalc` 파일(`compensation@1`)은 불러올 때 `compensation@2` envelope로 자동 마이그레이션되며, 부상 모드로 그대로 복원됩니다.
...
- 원화 입력 포맷을 `apps/desktop/src/lib/format-won.ts` 공용 헬퍼로 통합하고 이자 원금, 소송비용 금액 입력, 변제충당 금액 입력이 같은 콤마 표시·파싱 규칙을 사용하도록 정리했습니다.
- 소송비용 소가비례 분배 입력을 textarea 에서 pill/chip 입력으로 교체해 여러 당사자별 소가를 더 명확하게 검토할 수 있게 했습니다.
...
- 릴리스 dry-run 체크리스트에 이전 릴리스 post-mortem 기반 실제 `.app`/`.msi` 실행, 5개 export/save/load 버튼 sanity, case-001 `49,863` 결과 확인을 추가했습니다.
...
- 개발 워크플로, 릴리스 체크리스트, 테스트 전략, 사용자 테스트 계획 문서를 추가했습니다.
