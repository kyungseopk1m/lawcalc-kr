...
> **주의 — 같은 입력에 v0.11.0 과 다른 금액이 나옵니다.** 이번 변경은 정확성 수정이 대부분이라
> 이자·상속분·소송비용·변제충당·손해배상 전 도메인에서 결과가 달라질 수 있습니다. 진행 중인
> 사건에 v0.11.0 결과를 이미 제출했다면 다시 계산해 확인해 주세요. 버전 번호는 `0.x` 규칙상
> minor 로 올라가지만, 금액이 바뀌는 폭은 major 급입니다.

### Added

- 상속인마다 **상속포기**(민법 제1041조)를 체크할 수 있습니다. 포기는 대습원인이 아니므로(제1001조) 대습상속인이 생기지 않습니다. 어느 촌수 전원이 포기하고 배우자가 있으면 배우자 단독상속으로 처리합니다(대법원 2023. 3. 23.자 2020그42 전원합의체). 대습상속인 각자의 포기도 따로 넣을 수 있습니다.
- 대습이 생긴 **원인**을 상속개시 전 사망 / 상속결격(제1004조) / 상속권 상실선고(제1004조의2)로 구분해 입력합니다.
- 변제충당에 **변제일** 입력을 추가했습니다. 그 날짜를 기준으로 변제기 도래 여부를 판정합니다(제477조 제1호). 비워 두면 종전처럼 계산 시점을 기준으로 봅니다.
- 변제충당 결과에 **충당 순위**를 표시합니다. 변제기 도래 여부와 변제이익 순위, 동순위 비례 안분 여부가 채권마다 나옵니다.
- 소송비용에 **소가 산정 기준**을 추가했습니다. 소가를 산출할 수 없는 재산권상의 소와 비재산권을 목적으로 하는 소송은 간주 소가 5,000만원, 규칙 제15조①~③·제15조의2·제17조의2·제18조에 정한 소송은 1억원으로 계산합니다(「민사소송 등 인지규칙」 제18조의2).
- 항고·재항고 사건에 **원신청서 인지액** 입력을 추가했습니다. 제9조·제10조의 신청에 관한 재판에 대한 항고는 그 2배로 계산하고(인지법 제11조 제1항), 비워 두면 제11조 제2항의 2,000원 정액이 적용됩니다.
- 향후치료비·보조구에서 같은 지출을 여러 항목으로 나눠 넣은 것으로 의심되는 조합을 감지해 안내합니다. 수치합계 한도가 항목마다 따로 걸려 합계가 커지기 때문입니다. **금액은 바꾸지 않고 표시만 합니다.**
...
- 소촉법 이율을 골랐을 때 계산 기간이 이율 변경일(2015-10-01, 2019-06-01)을 걸치면 부칙 경과조치 안내를 띄웁니다. 변론종결일은 입력에 없으므로 계산은 그대로 두고 안내만 합니다.
- 저장하지 않은 계산 내용이 있는 상태로 창을 닫으면 확인 창이 뜹니다.
- 상단 탭을 스크린리더가 탭으로 인식하도록 바꾸고, 활성 탭을 색 외에 밑줄로도 표시합니다.

### Fixed

- **이자**: 여러 구간으로 나눈 계산에서 초일 불산입이 구간마다 중복 적용돼 구간 수만큼 일수가 빠졌습니다. 2구간 363일(정답 364일), 3구간 2,189일(정답 2,191일)이었습니다. 초일 불산입은 기간의 기산일에 한 번 적용하는 규칙이고(민법 제157조) 구간 분할은 이율 문제라 무관합니다.
- **이자**: 이율에 `NaN` 이나 무한대가 들어가면 검증 없이 통과해 이자가 `NaN` 으로 나왔습니다. 원금은 이미 막혀 있어 대칭이 깨져 있었습니다.
- **이자**: 소촉법 이율을 골랐는데 계산 기간이 2003-06-01 이전까지 걸치면 함수 이름이 든 영어 오류 문구가 결과 화면에 그대로 떴습니다. 이제 계산 전에 한국어로 안내하고 계산을 막습니다. 그 이전 기간에 소촉법 이율이 없는 이유는 데이터 누락이 아니라 헌재 2003. 4. 24. 선고 2002헌가15 위헌결정으로 구 규정이 실효됐기 때문이며, 안내 문구에 그 사유를 밝힙니다.
- **이자**: 민법·상법 법정이율의 적용 시작일이 시행일이 아니라 공포일로 들어가 있었습니다. 1958~1959년·1962년 기산 계산의 결과가 달라집니다.
- **이자**: 이자제한법 데이터셋의 대통령령 번호 3건을 정정했습니다.
- **상속**: 1순위 직계비속에 최근친 우선(제1000조 제2항)이 적용되지 않아, 자녀와 손자녀를 함께 넣으면 손자녀에게도 지분이 갔습니다.
- **상속**: 2026. 3. 17. 공포·시행된 법률 제21454호가 제1003조 제2항의 대습 배우자를 "사망 또는 결격된 자의 배우자"에서 "상속개시전에 사망한 사람의 배우자"로 좁힌 것을 반영합니다. 시행일 이후 개시된 상속에서 결격·상속권 상실이 원인이면 피대습자의 배우자는 대습분을 받지 못합니다. 직계비속 대습은 그대로 유지됩니다. 부칙 제2조의 적용례에 제1003조가 없어 시행일로 갈립니다.
- **상속**: 같은 상속인에게 "상속개시 전 사망"과 "상속포기"를 함께 체크하면, 입력해 둔 대습상속인이 경고 없이 결과에서 빠졌습니다(최악의 경우 배우자 단독상속으로 뒤집혔습니다). 두 상태를 함께 지정할 수 없게 했습니다.
- **소송비용**: 항소·상고 인지액이 불복 범위를 반영하지 않고 전체 소가로 계산됐습니다. 전체 소가 1억원·불복 범위 3,000만원 사건에서 3.25배로 과대했습니다(「민사소송 등 인지규칙」 제25조).
- **소송비용**: 항소·상고 사건을 저장하면 전체 소가가 파일에 남지 않아, 다시 열어 심급을 1심으로 되돌리면 불복 범위를 소가로 착각해 조용히 과소 계산됐습니다.
- **소송비용**: 조정신청(머)에 인지법 제2조 소장 산식이 그대로 적용돼 10배로 나왔습니다. 「민사조정규칙」 제3조의 10분의 1을 적용합니다.
- **소송비용**: 조정신청에 항소·상고 심급을 지정할 수 있어, 존재하지 않는 상소 수수료가 산출됐습니다(소가 3,000만원 기준 항소 21,000원·상고 28,000원). 조정 신청 수수료에는 심급 개념이 없어 1심으로 강제합니다.
- **소송비용**: 항고·재항고(라/마) 인지를 소장 누진식으로 계산했습니다. 인지법 제11조의 별도 체계를 적용합니다.
- **소송비용**: 전자소송 감액(제16조)이 1,000원 하한에 먹혀 무효화되던 구간이 있었습니다. 하한을 심급 배수보다 앞에 두도록 순서를 바로잡아, 소가 10,000원 사건의 항소가 1,000원(정답 1,500원)·상고가 1,000원(정답 2,000원)으로 나오던 것도 함께 고쳤습니다.
- **소송비용**: 송달료 회당 단가가 낡아 있었습니다(5,500원 → 5,640원).
- **소송비용**: 가압류·가처분 이의·취소 신청사건의 변호사보수가 0원으로 강제됐습니다. 규칙 제3조 제2항 단서의 변론·심문 요건은 신청사건 전용이라 이의·취소에는 적용되지 않습니다.
- **소송비용**: 소가 산정 기준 값에 오타가 들어가면 검증 없이 간주 소가로 처리돼, 소가 10억원 사건의 인지액이 4,055,000원에서 230,000원으로 바뀌었습니다.
- **소송비용**: 항고 사건의 원신청서 인지액에 음수·`NaN`·무한대를 넣어도 검증 없이 통과해, 음수 인지액이나 `NaN` 이 결과에 실렸습니다.
- **소송비용**: 화면을 초기화해도 소가 산정 기준이 그대로 남아, 기본 화면에서 230,000원이 계산됐습니다(정상 140,000원).
- **소송비용**: 불복 범위가 소가와 따로 움직여, 소가만 고치면 불복 범위가 옛 값에 남았습니다. 이제 비워 두면 소가를 따라갑니다. 간주 소가를 고르면 불복 범위 입력도 함께 비활성됩니다.
- **변제충당**: 동순위 채권의 비례 안분에서 변제금이 사라졌습니다. 변제 5원·총채무 6원이면 4원만 충당됐고(1원 증발), 5채권 74원은 71원이 됐습니다. 배분 결과가 채권 입력 순서에 따라 달라지기도 했습니다.
- **변제충당**: 채권 잔액에 안전정수 범위를 벗어난 값을 넣어도 검증 없이 통과했습니다.
- **손해배상**: 사고 당시 만 25세 미만이면 계산 자체가 불가능했습니다(만 24세·만 10세·신생아 모두 오류). 가동연한을 65세보다 높게 잡아도 같았습니다.
- **손해배상**: 가동연한이 이미 지난 피해자는 위자료만 청구해도 계산이 거부됐습니다.
- **손해배상**: 기왕증 기여도를 입력해도 일실수입에 반영되지 않았습니다. 입력란과 저장까지 배선돼 있는데 계산만 이 값을 읽지 않아, 조용히 과대한 금액이 나왔습니다.
- **손해배상**: 극단적으로 짧은 한시장해가 경고 없이 사라졌습니다.
- **손해배상**: 같은 지출을 나눠 넣은 조합을 감지할 때 기간이 부분적으로 겹치거나 한쪽이 다른 쪽에 포함되는 경우를 놓쳤습니다.
- 계산 결과 화면의 조문 표기에서 절 기호를 없애고 "제N조 N항"으로 통일했습니다.

### Changed

- 상속포기·대습 원인, 변제일, 소가 산정 기준·전체 소가·원신청서 인지액이 담긴 `.lcalc` 파일은 `inheritance@2` · `appropriation@2` · `litigation-cost@2` 를 요구합니다. 이 필드들이 없는 파일은 종전대로 `@1` 로 저장되어 이전 버전에서도 열립니다. 반대로 새 필드가 담긴 파일을 이전 버전에서 열면 필드가 조용히 버려져 금액이 달라지므로, 이전 버전이 열지 못하게 막습니다.
