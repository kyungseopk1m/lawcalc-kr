...
MVP는 판결금·지연손해금 이자 계산에 집중합니다. 사용자가 원금, 시작일, 종료일, 이자율 구간, 초일 산입 여부, 윤년 처리 방식을 입력하면 계산 엔진이 구간별 일수와 이자, 합계, 적용 공식을 반환하는 구조입니다.
...
| 구분               | W2 상태                           | 계획                                         |
| ------------------ | --------------------------------- | -------------------------------------------- |
| 판결금 이자 계산   | 타입 surface 준비                 | W2/W3 계산 엔진 연결                         |
| 지연손해금 계산    | 법정이율 프리셋 설계              | 민법 5%, 상법 6%, 소촉법 이율 변경 이력 반영 |
| 계산 근거 표시     | 결과 타입에 공식·데이터 버전 포함 | 결과 표와 PDF 리포트에 표시                  |
| `.lcalc` 저장/로드 | IPC/Rust command stub 준비        | 로컬 JSON 포맷으로 재현성 보장               |
| CSV/PDF/클립보드   | IPC/Rust command stub 준비        | W4 출력 구현                                 |
...
파일에는 최소한 `schemaVersion`, `appVersion`, `dataVersion`, `createdAt`, `input`, `options`, `result`, `disclaimer`가 포함됩니다. 새 버전에서 필드가 추가되더라도 기존 파일을 읽을 수 있도록 하위 호환을 유지합니다.
