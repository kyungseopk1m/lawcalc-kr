...
법원이 공개한 계산 프로그램은 윈도우 전용이라 맥·리눅스에서는 쓸 수 없습니다. 같은 계산을 맥에서도 하려고 만들었습니다.
...
결과를 청구취지 문구로도 만들어 줍니다. 법원 기재례를 따라 "원금 및 이에 대하여 ○○부터 다 갚는 날까지 연 ○%의 비율로 계산한 돈" 형식으로 나옵니다(이율이 여러 구간이면 "각 비율"). 마지막 구간을 "다 갚는 날까지"로 열어 둘지 계산 종료일로 닫을지 고른 뒤, 복사해 소장이나 지급명령 신청서에 붙이면 됩니다.
