...
export interface TreatmentResult {
  ...
  /**
   * 같은 지출을 두 항목으로 쪼갠 것으로 의심되는 조합이 있는지 (UI 안내).
   *
   * 수치합계 상한은 항목별로 걸리므로, 월 1회 30년을 월 1회 15년 두 건으로 나누면 상한
   * 여력이 두 배가 된다. 상한이 항목별인지 손해 항목 전체인지는 정책 판단이 필요하고
   * 정당하게 분리된 항목(다른 부위·다른 치료)을 합산하면 그것대로 오답이 되므로, 계산은
   * 바꾸지 않고 의심 조합만 표시한다.
   *
   * optional 이라 이 필드가 없는 구 `.lcalc` result 도 그대로 로드된다.
   */
