...
> **상태 (2026-05-10, v0.2.3)**: 공개 API 안정. core-engine 단위 회귀 115개 / 골든 게이트 21개 통과. 이자 엔진은 반올림 정책 v2 (`options.rounding`) + 데이터셋 주입 (`calculateInterest(input, { dataset })`) 을 지원하고, 상속분 엔진은 1991-01-01 이후 사망 케이스와 1차 대습상속까지 지원한다.
