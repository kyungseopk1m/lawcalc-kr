/**
 * Public domain types for the lawcalc-kr inheritance (상속분 간이) calculation engine.
 *
 * 근거: 외부 reference 매뉴얼 (private). 적용 조항을 직접 명시한다.
 * 적용 조항: 민법 제1000조, 제1001조, 제1003조, 제1009조, 제1010조.
 *
 * 적용 시점: 1991. 1. 1. 이후 피상속인 사망 케이스만 (1990 개정민법 시행).
 */

import type { IsoDate } from "../types";

/**
 * 상속인 노드. 4 순위 그룹의 원소이자 대습상속인의 원소.
 *
 * - `deceasedBeforeOpening`: 대습 원인 발생 여부. 사망뿐 아니라 결격·상속권 상실도 true 다.
 *   어느 원인인지는 `representationCause` 로 구분한다 (제1003조 제2항 개정 이후 금액이 갈린다).
 *   **필드명이 원인 중 하나만 가리켜 오해를 부른다.** 제1004조의2 제6항의 상속권 상실선고는
 *   상속개시 **후** 확정되고 개시시로 소급하므로 "상속개시 전" 이 아니다. 이름을 고치면
 *   저장된 `.lcalc` 이 열리지 않아 그대로 두고, 뜻은 "대습 원인이 있다" 로 읽는다.
 * - `renounced`: 상속포기. 사망·결격과 **다른 필드**다 — 포기는 대습 사유가 아니다.
 * - `representatives`: 1차 대습 상속인. v0.2 첫 PR 은 1차만 지원.
 *   재귀 type 으로 v0.3+ 에서 N차 대습 일반화 대비 — runtime 에서 depth ≥ 2 거부.
 */
export interface HeirNode {
  name?: string;
  deceasedBeforeOpening: boolean;
  /**
   * 상속포기 (민법 제1041조·제1042조). 포기자는 상속개시시부터 상속인이 아니었던 것으로 본다.
   *
   * `deceasedBeforeOpening` 으로 대용하면 안 된다. 제1001조의 대습원인은 "상속개시전 사망"
   * 과 제1004조·제1004조의2(결격·상속권 상실선고) 뿐이고 포기는 열거되어 있지 않다.
   * 포기를 사망으로 입력하면 있지도 않은 대습이 발생해 손자녀에게 지분이 배분된다.
   *
   * 미지정 시 기존 동작과 완전히 동일하다 (회귀 0).
   */
  renounced?: boolean;
  /**
   * 대습 원인 (`deceasedBeforeOpening === true` 일 때만 의미가 있다).
   *
   * - `"death"`: 상속개시 전 사망
   * - `"disqualified"`: 상속결격 (제1004조)
   * - `"forfeited"`: 상속권 상실선고 (제1004조의2)
   *
   * 제1001조의 직계비속 대습은 세 원인 모두에서 발생한다. 그러나 제1003조 제2항이
   * 2026. 3. 17. 개정으로 "사망 또는 결격된 자의 배우자" 에서 **"상속개시전에 사망한
   * 사람의 배우자"** 로 좁아져, 시행일 이후 개시된 상속에서는 결격·상실의 경우 피대습자의
   * 배우자가 대습 슬롯을 받지 못한다. 그 분기에만 쓰인다.
   *
   * 미지정 시 `"death"` 로 본다 — 필드명이 사망을 뜻하고, 개정 전 동작과 같다.
   */
  representationCause?: "death" | "disqualified" | "forfeited";
  representatives?: HeirNode[];
  /**
   * 촌수 (직계비속: 자 1·손 2 / 직계존속: 부모 1·조부모 2 / 방계 4순위: 삼촌 3·사촌 4).
   * 같은 순위 안에서 촌수가 다르면 최근친(최소 촌수)만 상속한다 (민법 제1000조 제2항).
   * 미지정 시 그룹 전원을 동순위로 보고 균분한다 (하위호환 — 기존 입력·`.lcalc` 무영향).
   * 형제자매에는 적용하지 않는다 (전원 2촌이라 최근친 판정이 무의미하다).
   */
  degree?: number;
  /**
   * 이 대습상속인이 피대습자의 배우자(며느리·사위)인지. 제1003조 제2항 대습 + 제1009조 제2항 5할 가산.
   * `representatives` 안에서만 의미가 있다. 미지정 시 직계비속 대습자로 보고 균분한다(하위호환).
   */
  isSpouseOfRepresented?: boolean;
}

/**
 * 상속분 계산 입력.
 *
 * 4 순위 분리 필드 (직계비속/직계존속/형제자매/방계4촌) + 배우자 단수.
 * 상위 순위가 1명 이상 입력되면 하위 순위는 자동으로 무시하는 선순위 우선 정책을 따른다.
 *
 * 한국어 라벨 출처: source-extraction-spike-2026-05-09.md 8.3절 (UI strings verbatim).
 */
export interface InheritanceInput {
  decedent: {
    name?: string;
    /** 1991-01-01 이후 강제. 그 이전이면 RangeError throw. */
    deceasedAt: IsoDate;
  };
  /**
   * 배우자 0~1. 매뉴얼 + UI strings 가 single object 로 hard-cap (`배우자를 한명 이상 입력할 수 없습니다`).
   * `alive: false` 또는 omit 양쪽 모두 "배우자 미존재" 동치.
   */
  spouse?: { name?: string; alive: boolean };
  /** 1순위 직계비속. 매뉴얼 제3조 제2항 fall-through. */
  linealDescendants?: HeirNode[];
  /** 2순위 직계존속. 1순위 부재 시에만 분배 참여. 대습 X (1001조). */
  linealAscendants?: HeirNode[];
  /** 3순위 형제자매. 1·2순위·배우자 모두 부재 시에만. 대습 가능 (1001조). */
  siblings?: HeirNode[];
  /** 4순위 4촌 이내 방계혈족 (1000조 ①-4). 1·2·3순위·배우자 모두 부재 시에만. 대습 X. */
  collateralFourth?: HeirNode[];
}

/**
 * 상속인별 지분.
 *
 * 분수 표기 정원 (UI strings `1/1`, `[배우자] (1X1)` 와 정합).
 * - `numerator/denominator`: GCD 약분 후 정수 쌍.
 * - `rawNumerator/rawDenominator`: 약분 전 분수 (검산·디버그용).
 */
export interface InheritanceShare {
  name: string;
  numerator: number;
  denominator: number;
  rawNumerator: number;
  rawDenominator: number;
}

/**
 * 상속분 계산 결과.
 *
 * `dataVersion` 은 inheritance 도메인 module-level 버전 식별자
 * (예: "inheritance/v1.0.0"). 비율 4 상수 + 1991-01-01 cutoff 정원이 변경되면 bump.
 */
export interface InheritanceResult {
  decedent: { name?: string; deceasedAt: IsoDate };
  shares: InheritanceShare[];
  /** B11 단일 source — `STANDARD_DISCLAIMER` 그대로. */
  disclaimer: string;
  dataVersion: string;
  /** ISO 8601 datetime. */
  computedAt: string;
}
