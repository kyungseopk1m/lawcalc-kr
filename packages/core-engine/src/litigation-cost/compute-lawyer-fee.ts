...
import { validateKlacDiscountScope, validateLawyerFeeInput } from "./validators";
 ...
 *   4. applyLawyerFeeDiscounts(baseAmount, discounts, klacAgreedFeeWon)  — PR 1 helper.
 *      → { amountWon, multiplier (clamp 후), rawMultiplier, clamped }.
 *      누적 (compound) + clamp [0.0, 1.5] (제6조 ②항 cap).
 *   5. validateKlacDiscountScope(caseType, discounts)  — 비차단 warning. result.klacWarnings 노출.
...
function discountSegment(discount: LawyerFeeDiscount): string {
  switch (discount.kind) {
    ...
    case "klac":
      return "KLAC (대한법률구조공단, ×0.42 default)";
...
/**
 * 변호사보수 계산. 입력 검증 → bracket 매칭 → 누진 산식 → 감액/조정 누적 → KLAC scope warning.
 *
 * @example
 ...
 *   // r.klacWarnings.length === 0 (민사 — 적용 범위)
 */
export function computeLawyerFee(
  ...
  const { amountWon, multiplier, rawMultiplier, clamped } = applyLawyerFeeDiscounts(
    baseAmount,
    input.discounts,
    input.klacAgreedFeeWon,
  );

  const klacWarnings = validateKlacDiscountScope(input.caseType, input.discounts);
