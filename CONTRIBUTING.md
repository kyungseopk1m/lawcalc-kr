...
3. CHANGELOG 의 해당 버전 섹션이 한국어 사용자 노트로 채워졌는지 (core 엔진 변경, 데스크톱 UI 변경, `.lcalc`·CSV·PDF·클립보드 변경, 법정이율 데이터 변경, 스키마/계산 정책 호환성 변경).
4. README 의 데모 자산이 placeholder 가 아닌 실제 자산인지.
5. v0.2+ 미완성 기능은 UI 에서 비활성화되었거나 차후 버전으로 표시되었는지.
...
사용자 (변호사·법무 실무자) 피드백은 GitHub Issues 로 받습니다. 이용자 대부분이 법률 실무자라 GitHub 계정이 장벽이 되므로, 앱의 정보 다이얼로그에 메일 제보 경로를 따로 두었습니다. 다음 항목이 포함되면 빠르게 분류할 수 있습니다.
