...
데이터셋은 `data/<이름>/v{N}.json` 으로 관리하고, `packages/*/scripts/sync-*.mjs` 가 이를 `*.dataset.generated.ts` 로 인라인합니다. JSON 을 고쳤으면 `pnpm --filter <패키지> sync:datasets` 로 재생성한 결과까지 함께 커밋합니다. 결과 객체에는 항상 `dataVersion` 을 기록해 동일 입력의 재현성을 보장합니다. 데이터 변경은 CHANGELOG 에 사용자 영향 1~2 줄을 함께 적어 주세요.
...
> 실제 사례. `legal-rates` 1.0.1 은 민법·상법 법정이율 슬라이스를 공포일에서 시행일로 옮기면서(`1958-02-22` → `1960-01-01`, `1962-01-20` → `1963-01-01`) patch 를 썼습니다. 1958~1959·1962 기간을 쓰는 계산의 동작이 바뀌므로 규칙상 minor 였습니다. 다만 이 번호는 v0.12.0 으로 이미 공개돼 그대로 둡니다.

버전을 올릴 때는 그 태그를 참조하는 곳을 함께 갱신합니다. 골든 픽스처의 `dataVersion`, 단위 테스트의 태그 문자열, README 와 `docs/ARCHITECTURE.md` 의 예시입니다. `pnpm test:golden` 이 어긋남을 잡아 줍니다.
